...
static int atoslng(char *s, int32 *slng);
...
static int swisseph(char *buf)
{
  ...
  int nast, iast;
  int32 astno[100];
  int32 iflag = 0, iflag2;              /* external flag: helio, geo... */
  int32 iflgret;
  int32 whicheph = SEFLG_SWIEPH;
...
#endif
  if ((int32) pd.year * 10000L + (int32) pd.mon * 100L + (int32) pd.mday < 15821015L) 
    gregflag = FALSE;
  else
    gregflag = TRUE;
  jday = pd.mday;
...
static int atoslng(char *s, int32 *lng)
{
  char *sp;
  for (sp = s; *sp != '\0'; sp++) {
    if (!isdigit(*sp) && *sp != '-') {
