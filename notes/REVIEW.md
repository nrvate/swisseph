...
- **Planetary moons: a context's first answer differs from its later ones
  (2026-09-16 review, F9).** `swe_calc(2451545.5, 9401, SWIEPH|SPEED)`
  gives longitude `0x1.485a29aae08ffp+8` on a fresh context and
  `0x1.485a29aadfc7bp+8` from the second call on (~1e-10 degrees; speeds
  move more). Upstream gives identical bits. Bisected on ts.12: a prior
  calculation of the Earth, Mars, the Sun, the Moon or `SE_ECL_NUT` at the
  same instant does NOT change the first answer; only a prior call for the
  same moon does -- so the state is the moon file's own (its save area,
  segment, or the center-of-body `sweph()` calls in `main_planet()` and
  `app_pos_etc_plan()`), not a shared cache. Deferred 2026-09-16: bounded
  size, planetary moons only, and root cause not isolated in the time the
  review allowed.
