...
Restored by it today: the JPLHOR cache-key contamination,
`swe_calc_pctr()`'s missing keying, and (ts.14) `SE_ECL_NUT` wiping the open
files' constants and `swe_pheno(SE_ECL_NUT)`'s out-of-bounds read, all
described in Closed; sweph.h keeps the list. Every `#ifndef
SWE_UPSTREAM_COMPAT` block is by construction a fix upstream does not have,
which makes the switch the inventory for notes/UPSTREAM-BUGS.md.
...
*(2026-09-16: the resolution's inputs changed under this note — the DE
number now comes from the configuration the setters publish
(`sweph_denum_moon`/`jpldenum_cfg`, Closed below) whenever the files are
closed, so the open-state term of the proposed key is gone. The cost
question stays open exactly as written; the order-dependence question does
not — that was never this memo's to cover, and is now Closed.)*
...
- ~~**`bessel()`**~~ — done, see Closed. It needed a real DE ≥ 403; **G25**
  `check-jplhor` supplies one opt-in and reaches it, and the first execution
  of that path segfaulted.
...
| `swe_close()` freed `ctx->dpsi`/`ctx->deps` and nulled them but left `ctx->eop_dpsi_loaded` set. `load_dpsi_deps()` opens with `if (eop_dpsi_loaded > 0) return;` — a load-once guard — so the next JPL Horizons request reloaded nothing and `bessel()` dereferenced NULL. **`swe_calc(…SEFLG_JPLHOR); swe_close(); swe_calc(…SEFLG_JPLHOR);` segfaults** | the flag is reset alongside the arrays it describes. Unreachable until now because `SEFLG_JPLHOR` survives `plaus_iflag()` only with a JPL file of DE ≥ 403 *and* the IERS corrections loaded — `de200.eph` is shipped but is DE200, and G19's synthetic header carries `numde = 431` with no coefficients, so each supplies half. **G25** `check-jplhor` supplies both and reaches it; `bessel()` 0.00% → 78.05%, having never executed in the life of this fork. Opt-in, because the file is 2.6 GB and cannot be committed or checked out in CI; skips loudly without one. Proven by reverting the reset, which segfaults the gate |
| `leapsec_done` and `init_dt_done` survived `swi_close_keep_topo_etc()`, which is what `swe_set_ephe_path_r()` calls when the ephemeris path is being replaced. Both guard a table read from that path, so `init_leapsec()`/`init_dt()` returned early and the OLD directory's tables carried into the new one — a second path's `seleapsec.txt` was simply never read | reset alongside the seorbel line cache one line above, which is dropped there for exactly this reason and was the precedent these two were missing. Same class as the `eop_dpsi_loaded` crash, found by sweeping for load-once flags after it. Pinned by a `path switch` assertion in **G23**; proven by reverting the reset. `init_dt()` clears the delta-t memo itself on re-run, so the flags are all that had to go back — the delta-t half is fixed by inspection and the same reset, not by an observed reproducer |
| The tidal term behind delta-t read **which files a context happened to have open** — upstream's `swi_get_tid_acc()` denum guards (notes/UPSTREAM-BUGS.md section 14), the same family as the two tid_acc leaks closed before it but a different member: those propagated a known denum down wrong paths, this one never knew it until a file was open. A fresh `swe_ctx_new()` child — which inherits the configuration but not the asking context's open files — answered its first pre-1955 delta-t with `SE_TIDAL_DEFAULT` (−25.8) and every later one with the file's term (DE441's −25.936), so the same body at the same instant moved with call order: the Moon at JD 2415020.5 answered **272.41632607** fresh and **272.41633228** after a Mercury calculation on the same context, ~0.05 arcsec apart. Invisible at J2000, where the delta-t polynomial carries no tidal term — which is why every modern-date test was blind to it; found by the ephemeris server's golden gate, which asks a fresh context at 1900 | the DE numbers the setters' header pre-opens read are now **configuration**: `sweph_denum_moon` published by `swe_set_ephe_path_r()`, `jpldenum_cfg` by `swe_set_jpl_file_r()`, both in `SWI_CFG_PATH`, inherited by every context, read by `swi_get_tid_acc()` and `calc_deltat()` whenever the files are closed. Zero means unknown and the default term is then the honest answer for every call, first or fiftieth. Property 5 of **G4** `check-ctx`: one context, several bodies at JD 2415020.5, every answer bit-equal to the brand-new-context answer, delta-t included — falsified by reverting either resolution site (it takes BOTH to reintroduce the drift, because `calc_deltat()` passes its denum to `swi_set_tid_acc()`, which re-resolves it: reverting either one alone leaves determinism intact). G1 bit-identical, no transcript row moved |
| The 2026-09-16 review of ts.11 (the Astrolog ephemeris server project's `EPHEMERIS_REVIEW.md`, F1-F11). **F1** ts.11's delta-t fix substituted the published moon DE number in `calc_deltat()` BEFORE resolution, so `JPLEPH\|SWIEPH` took DE441's tidal term instead of the JPL file's DE200 -- 0.56 s of delta-t, 0.054" on a centered position through the ephemeris server. **F2** `swe_close()` kept the two published DE numbers, where upstream's close forgets them; G8 failed setest 8.2.1, 8.4.1, 8.6.1, 8.10.1. **F3** a default-context thread missed a DE-number change under an unchanged path string. **F4** the JPL DE number was published by every `open_jpl_file()`, lazy opens on calculation paths included, and survived a path change. **F5** the Moshier early return in `swi_get_tid_acc()` (moved out of the `denum == 0` guard earlier on this fork) ignored the JPL > Swiss > Moshier precedence: `MOSEPH\|SWIEPH` took DE404, 0.098 s from upstream. **F6** `swe_nod_aps_r(SE_SUN, TOPOCTR, MEAN)` read an unfilled Earth in `swi_deflect_light()`: NaN on a fresh context, history-dependent after (upstream hides it behind its path-setter Moon calculation). **F7** `swe_calc_pctr_r()` under TOPOCTR deflected with whatever observer an earlier call saved. **F10** `swi_get_observer()` reused obliquity/nutation keyed on the epoch alone. **F11** past a file's nominal range a fresh context failed where a context with the neighbouring file open answered (upstream's structure: an open file serves its whole data range, a fresh lookup tries only the bucket's name) | **F1** the `calc_deltat()` substitution removed; the resolver's Swiss fallback applies only while no DE number is found. **F2** `ctx_release()` clears both. **F3** `path_changed` compares both. **F4** only `swe_set_jpl_file_r()` publishes; a path change clears it. **F5** the Moshier return requires Moshier alone. **F6** the save-area calculation for the Earth runs without TOPOCTR. **F7** `swe_calc_pctr_r()` computes and saves the observer at its instant. **F10** the shortcut checks `SWI_EPS_KEY` on both caches. **F11** a missing bucket file tries the neighbouring bucket's, the range check still refusing an instant it does not cover. F6, F7 and F11 are behind `SWE_UPSTREAM_COMPAT`. Property 6 (REVIEW) of **G4** `check-ctx` holds F1, F4, F5, F6, F7, F11 as relations on the fork alone; F2 is G8's. F8 -- a `SE_SIDBIT_PREC_ORIG` sidereal mode's precession models persisting in a context -- is upstream's semantics and is guarded in the ephemeris server, which resets models before the next request |
| **F9** (2026-09-16 review, deferred the same day, closed on ts.13). `swe_calc(2451545.5, 9401, SWIEPH\|SPEED)` answered `0x1.485a29aae08ffp+8` on a fresh context and `0x1.485a29aadfc7bp+8` from the second call on; upstream does the same. Root cause: 2451545.5 is exactly a boundary of Phobos's 4-day Chebyshev segments. A fresh context loads the segment `get_new_segment()` computes, the one STARTING there; the first call's light-time step at `tjd - dt` then loads the previous one, and `sweph()`'s inclusive range test (`tseg0 <= tjd <= tseg1`) kept it for every later call. Adjacent fits agree only to ~1e-10 degrees at the join. Not a moon-file state, as the bisect had suggested -- any file's boundary instant has the same shape, planets included | `sweph()` reloads when the loaded segment is not the one `get_new_segment()`'s arithmetic names (except at the file's last instant, whose index is one past the last segment), behind `SWE_UPSTREAM_COMPAT`. **G4** `check-ctx` property 6 holds it (fresh; a call one day earlier; a repeat), and fails on the upstream rule. G1, G8, G24 unchanged |
| **S-fork** (Astrolog ephemeris server review, 2026-09-16; closed on ts.14). The server's fuzz had UBSan report `ipl * 100` overflowing in `swe_calc()`'s center-of-body setup and a double-to-int overflow in `get_new_segment()`; the server bounded its wire ids and left the library alone. A fuzz here found the second was not about hostile ids: `swe_calc(SE_ECL_NUT)` under a different ephemeris flag ran `free_planets()` outside the `ipl != SE_ECL_NUT` exemption, so the open files kept `fptr` and lost `tfstart`/`dseg`, and Sun (Swiss), SE_ECL_NUT (Moshier), Mars (Swiss) returned ERR "sepl_18.se1 is damaged" after dividing by `dseg = 0`. The same fuzz found `swe_pheno(SE_ECL_NUT)` reading `pla_diam[-1]` and `mag_elem[-1]` and returning OK. All three present in upstream (notes/UPSTREAM-BUGS.md 15-17) | Behind `SWE_UPSTREAM_COMPAT`: `free_planets()` moved inside the exemption; the center-of-body test bounded at `ipl >= SE_ECL_NUT`; `swe_pheno()` refuses `ipl < SE_SUN`. **G24** `check-compat` gained the two observable ones in both directions; new **G25** `check-hostile` runs every calculation entry point over edge ids and flag sets plus a fixed-seed random walk under `-fsanitize=undefined,bounds-strict,float-cast-overflow` with no recovery, and fails on the compat build (400000 random calls clean on the fix). G1, G8 unchanged |
| **CI red from ts.12 to ts.14, and ts.14 released** (2026-09-17). Every push to main from b8e3355 on failed CI's compile matrix: `tests/ctxtest.c` read `dtFirst` after a branch gcc at -O1 and up could not relate to where it is set, and warned maybe-uninitialized under -Werror. The local `make -C tests check` compiles the tests at -O0 only, so it stayed green while CI was red for three releases' worth of commits | `dtFirst` initialised and its two checks guarded by whether the reference context existed (47671e5); CI's whole matrix -- gcc and clang, -O0 to -O3 and -Os, every library source, sample and test -- compiled clean locally before the push, then all 15 CI jobs passed. Tagged `v2.10.03-ts.14` on that commit (the library identical to d1779fe's); the release published four platform archives and SHA256SUMS, the Linux one checksum-verified and run. Before pushing to main: run that matrix, not only `make check` |
