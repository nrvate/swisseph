...
static void coverage(void) {
   ...
   * where a worker would adopt it mid-suite. */
  {
    ...
    *serr = 0; rf = swe_calc_ut(DATES[0], SE_SUN, SEFLG_SWIEPH|SEFLG_SPEED, x, serr);
    ...
    /* ⛔ Same shape, third instance: swe_calc_pctr() read the obliquity and
     * nutation caches without keying them to its own tjd first, so its answer
     * depended on the epoch of whatever had been computed before it -- 1.9
     * arcsec after a position 100 days away, 24.5 arcsec at 10000 days. The
     * two rows below are identical by construction and worthless if they ever
     * stop being, which is what makes them a test. */
